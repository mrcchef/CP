...
struct SegTree{
    ...
    void change(const int id,const long long val){ // change function
        // v[id].color += v[id].len*val;
        lazy[id].color = val;
    }
    void push(const int id){
        if(lazy[id].color!=0)
        {
            change(2*id,lazy[id].color);
            change(2*id+1,lazy[id].color);
        }
        lazy[id].color=0;
    }
    ...
    node query(int id,const int l,const int r,const int s,const int e){
        ...
        push(id);
        query(2*id,l,r,s,(s+e)/2)
        query(2*id+1,l,r,(s+e)/2+1,e);
...
void solve()
{
    ...
    for(i=n-1;i>=0;i--)
    {
        ...
        auto it2=st.upper_bound(vp[i].second);
