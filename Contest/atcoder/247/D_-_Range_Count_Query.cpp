...
struct node{
    ...
    node(ll _l,ll _r,ll _in,ll _val)
    {
        l=_l;
        r=_r;
        in=_in;
...
class MosAlgo{
    ...
    MosAlgo(ll _q,vector<node> _query,vector<ll> _arr)
    {
...
void solve()
{
    ...
    fo(i,q)
    {
        ...
        r--;
        // debug(mp(l,r));
        query[i]=node(l,r,i,val);
    }

    sort(all(query),compare);

    MosAlgo mos(q,query,v);
