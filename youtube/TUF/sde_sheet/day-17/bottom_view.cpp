...
//Similar to top view, now we simply need the last element at that
// horizontal distance from the node
// Time Complexity = O(nlogn)
// We have used modified Preorder traversal
map<ll,tree*> hm;
void bottomViewRecursive(tree *root,ll dis)
{
    if(root==NULL)
        return;
    // change code in top view and bottom view
    hm[dis]=root;
    
    bottomViewRecursive(root->left,dis-1);
    bottomViewRecursive(root->right,dis+1);
    
}

void bottomView(tree *root)
{
    bottomViewRecursive(root,0);
