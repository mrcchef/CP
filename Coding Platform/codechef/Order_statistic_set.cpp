...
void solve()
{
    ...
    fo(i,q)
    {
        ...
        if(ch=='I')
        {
            // ll isPresent=fn.rsum(mappedNum,mappedNum);
            // debug(isPresent);
            // if(isPresent)
            //     continue;
            if(freq[mappedNum]==1)
        ...
        {
            ll lessThanNum=fn.sum(mappedNum-1);
            cout<<lessThanNum<<endl;
        }
        else if(ch=='K')
        {

            ll totalNum=fn.sum(mxEle);
            ...
            else
            {
                ...
                {
                    ll mid=(st+ed)/2; 
                    ll lessThanMid=fn.sum(mid);
                    debug(mp(mid,lessThanMid));
                    if(lessThanMid<=num)
                    {
                        ans=mid;
                        st=mid+1;
