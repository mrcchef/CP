...
void solve()
{
    ...
    vi ans(q);
    // now queries are in increasing order of k
    // and since, we wanted to find out the number of elements 
    // greater than k, i.e. the number of elements that are smaller
    // than k1 will also be smaller than k2 (k1<k2)
    // therefore we build the segtree, and initially all the values are
    // one and for values of k, the elements that are smaller than k
    // we will update it's contribution to 0  
    // debug(vp);
    fo(j,q)
    {
        // debug(quries[j].k);
